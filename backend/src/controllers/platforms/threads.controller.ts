...
import { updateThreadsDetails } from "../../repositories/platform.repository";
...
export const connectThreads = (req: Request, res: Response) => {
    ...
    new SuccessResponse("Threads auth URL generated", { data: { url: authUrl } }).send(res);
};
...
export const threadsCallback = async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.redirect(`${ENV.APP.FRONTEND_URL}/settings?success=threads_connected`);

    } catch (error) {
        logger.error("Threads Connect Error", error);
        res.redirect(`${ENV.APP.FRONTEND_URL}/settings?error=threads_connect_failed`);
    }
};
...
export const disconnectThreads = async (req: Request, res: Response) => {
    try {
        ...
        await updateThreadsDetails(req.auth.id, { connected: false, platformId: null, auth: {}, profile: {} });
